...
--
CREATE
DATABASE IF NOT EXISTS `final` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
USE
`final`;
...
ALTER TABLE `answers`
  ...
  ADD CONSTRAINT `fk_answer_user` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON
DELETE
...
ALTER TABLE `picked_static_options`
  ...
  ADD CONSTRAINT `fk_pickedStaticOption_optionStatic` FOREIGN KEY (`static_option_id`) REFERENCES `static_options` (`id`) ON
DELETE
...
--
DROP
EVENT IF EXISTS `clearOldAccessTokens`$$
CREATE
DEFINER=`Tim5`@`%` EVENT `clearOldAccessTokens` ON SCHEDULE EVERY 1 MINUTE STARTS '2024-04-28 22:00:00' ON COMPLETION NOT PRESERVE ENABLE DO
UPDATE `user`
